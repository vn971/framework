      ...
      bindList.map(b => (b, b.css.openOrThrowException("Guarded with test before calling this method").
        ...
        case (elem, (bind, AttrAppendSubNode(attr))) => {
          ...
          if (calced.isEmpty) {
            ...
            new Elem(elem.prefix, elem.label, newAttr, elem.scope, elem.minimizeEmpty, elem.child: _*)
        ...
        case Full(todo: WithKids) => {
          ...
          calced.length match {
            ...
            case _ if id.isEmpty =>
              calced.map(kids =>
                new Elem(realE.prefix, realE.label, realE.attributes, realE.scope, realE.minimizeEmpty, todo.transform(realE.child, kids): _*))
            case _ => {
              ...
              calced.toList.zipWithIndex.map {
                case (kids, 0) =>
                  new Elem(realE.prefix, realE.label, realE.attributes, realE.scope, realE.minimizeEmpty, todo.transform(realE.child, kids): _*)
                case (kids, _) =>
                  new Elem(realE.prefix, realE.label, noId, realE.scope, realE.minimizeEmpty, todo.transform(realE.child, kids): _*)
              }
            }
          }
        }
          ...
          calced.length match {
            ...
            case 1 => {
              calced.head match {
                case Group(g) => g
                case e: Elem =>
                  new Elem(e.prefix, e.label, mergeAll(e.attributes, false, x == Full(DontMergeAttributes)), e.scope, e.minimizeEmpty, e.child: _*)
                case x => x
              }
            }
  ...
  final private def treatElem(e: Elem, onlySel: Boolean, depth: Int): NodeSeq = {
    ...
    if (onlySel) {
      ...
      lb.toList.filterNot(_.selectThis_?) match {
        case Nil => new Elem(e.prefix, e.label, e.attributes, e.scope, e.minimizeEmpty, run(e.child, onlySel, depth + 1): _*)
        case csb =>
          ...
          csb.partition(_.attrSel_?) match {
            ...
            case (attrs, Nil) => {
              ...
              new Elem(elem.prefix, elem.label, elem.attributes, elem.scope, elem.minimizeEmpty, run(elem.child, onlySel, depth + 1): _*)
            }
