  ...
  "lift <head> merger" should {
    "merge /html/body//head into existing /html/head section" in {
      ...
      susfiles match {
        case Full(sus) =>
          ...
          mergeToHtmlHead(actual).toString.replaceAll("\\s", "") must_== (expected.toString.replaceAll("\\s", ""))
        case _         =>
    ...
    "merge <lift:tohead> into a new head if not previously exist" in {
      ...
      susfiles match {
        ...
        case _         => failure("Failed loading test files") // TODO: Improve error message
      }
    }
   }
  ...
  /*
   "lift head cleaner" should {
     "remove duplicate title tag" >> {
       ...
       HeadHelper.cleanHead(actual) must equalTo(expected).ignoreSpace
     }
     "remove script tag with same id as previous script tag" >> {
       ...
       HeadHelper.cleanHead(invariant) must equalTo(invariant).ignoreSpace
     ...
     "remove script tag with src attributes if src attributes are equals to previous script" >> {
       ...
       HeadHelper.cleanHead(actual2) must equalTo(expected2).ignoreSpace
     }
