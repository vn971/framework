...
object MenuDslSpec extends Specification {
  ...
  "The Menu DSL" should {
    "allow basic menu definition via '/ path'" in {
      ...
      menu.loc.link.uriList must_== List("foo") 
      menu.loc.link.matchHead_? must_== false
    }

    "allow wildcard menu definitions via '/ path / **'" in {
      val menu = (Menu("Test") / "foo" / **).toMenu
      menu.loc.link.uriList must_== List("foo")
      menu.loc.link.matchHead_? must_== true
    }
    ...
    "handle LocParams" in {
      ...
      menu1.toMenu.loc.params.exists(_ == worthlessTest) must_== true
      menu2.toMenu.loc.params.exists(_ == worthlessTest) must_== true
    }
    ...
    "handle sub-submenus" in {
      ...
      menu.toMenu.kids(0).kids.size must_== 2
    }

    "handle I18N menu names" in {
      val menu = Menu.i("Home") / "index"

      menu.toMenu.loc.name must_== "Home"
    }
  }
